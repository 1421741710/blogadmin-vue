...
export const createWebSocket = (url,onMessage) => {
    ...
    //实时统计
    function handleRealTimeStatisticsEniry(message){
        onMessage('RealTimeStatistics',message)
    }
    ...
    ws.addEventListener('message',(event)=>{
        ...
        switch (da.type){
            ...
            case 'USER_LOGGED_OUT': //登出
                ...
                break;
            case 'LOG_ENTRY':   //日志
                handleLogEniry(da.message);
                break
            case 'Real_Time_Statistics':   //实时统计
                handleRealTimeStatisticsEniry(da.message);
                break
            default:
                return
        }
